...
const assignUsersIsFollowing = async (data, userId) => {
  ...
  try {
    usersOnfollowing = await FollowersFollowing.getCollection("followers-following")
      .aggregate([
        { $match: { userId: userId } },
...
const checkFollowersFollowing = async userId => {
  let userFollowersFollowing;

  try {
    userFollowersFollowing = await FollowersFollowing.getCollection("followers-following").findOne({
      userId: userId
    });
  } catch (error) {
    throw new HttpError("could not found the user's document, something went wrong.", 500);
  }
  ...
  if (!userFollowersFollowing) {
    ...
    try {
      ...
      throw new HttpError("could not create a collection in the database, please try again.", 500);
    }
  }
};
...
const getPsts = async (filter, { postsOp, quantity, scrollNo }) => {
  ...
  if (postsOp) {
    getInfoPerPage = [{ $skip: (+scrollNo - 1) * +quantity }, { $limit: +quantity }];
  }
